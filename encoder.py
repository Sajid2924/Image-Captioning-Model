...
class ImageEncoder(nn.Module):
    ...
    def __init__(self, frozen: bool = cfg.encoder_frozen):
        ...
        if frozen:
            ...
            ):  # frozen = true always in the config, so this loop will always run and will freeze all the parameters by setting requires_grad to False.
                param.requires_grad = False
            print(f"[Encoder] CNN weights FROZEN — will not be updated during training")
        else:
            print(
                f"[Encoder] CNN weights TRAINABLE — will be fine-tuned"
            )  # never set to False in this project.
    ...
    def forward(self, images: torch.Tensor) -> torch.Tensor:
        ...
        features = self.feature_extractor(
            images
        )  # self.feature_extractor = ResNet layers
        # Shape: (batch, 2048, 1, 1)

        # Flatten the spatial dimensions → (batch, 2048)
        features = features.squeeze(-1).squeeze(
            -1
        )  # (batch, 2048,1,1) -> (batch, 2048) by removing the last two dimensions of size 1.
