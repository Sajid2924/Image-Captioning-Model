...
class GPT2(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            TransformerBlock(n_embd, n_head, max_pos)
...
#     We match our layer names to HuggingFace names and copy
# ─────────────────────────────────────────────────────────────
    ...
    for our_key in our_sd.keys():
        ...
        if hf_key in hf_sd:
            ...
            if hf_tensor.shape == our_tensor.shape:
                ...
                print(f"  [skip] shape mismatch: {our_key} {our_tensor.shape} vs {hf_tensor.shape}")
                skipped += 1
        else:
            skipped += 1
